function dogleg_trust_region()
    ...
    % Models 1--4 set the system up close to the corresponding local minima listed above.
    m = 20;
    rng(6);
    model = 0;
    ...
    Delta = 1;
    I = eye(length(x));
    B = eye(size(g, 1), size(g, 1));

    
    while norm_g > tol && iter < iter_max
        % Solve the constrained minimization problem mmmmm
        B = 0.5 * (B + B');
        flag_boundary = 0;
        ...
        if eval_min > 0 % B is SPD: B = R' * R, R' * R * p = -g
            ...
            if p_norm >= Delta
                % Else: we are done with solving the subproblem
                p = p_U * Delta / p_norm;
                flag_boundary = 1;
            else
                p_b = -B \ g;
                ...
                if p_norm > Delta
                    alpha = roots([norm(p_b - p_U)^2, 2 * p_U' * (p_b - p_U), norm(p_U)^2 - Delta^2]);
                    p = p_U + max(alpha) * (p_b - p_U);
                    flag_boundary = 1;
                end
            end
        else
